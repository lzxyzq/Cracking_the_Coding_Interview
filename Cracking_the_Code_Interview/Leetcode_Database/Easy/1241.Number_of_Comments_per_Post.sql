...
-- 所有帖子的 parent_id 为 null

select 
    a.post_id,
    coalesce(b.num,0) as number_of_comments
from 
(select 
    distinct sub_id as post_id
from Submissions where parent_id is null) a
left join 
(select 
    parent_id,
    count(distinct sub_id) as num
 from Submissions group by parent_id) b 
